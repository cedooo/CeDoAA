...
def getFreeSSFromFile(): 
    p = Path('C:\greenSoftware')
    q = p / 'ssacount.txt'
    servers = []
    with q.open(mode='r',encoding='UTF-8') as f:
        for le in f:
            if le != '':
                serverObjAttrs = le.split(' ')
                ip = serverObjAttrs[1]
                port = serverObjAttrs[2]
                passwd = serverObjAttrs[3]
                method = 'aes-256-cfb'# serverObjAttrs[startIndex+3] #aes-256-cfb or
                tm = serverObjAttrs[5]
                country = serverObjAttrs[6]
                sjson = {"server": ip, "server_port": int(port), "password": passwd, "method": method, "remarks": "[" + time.strftime("%Y-%m-%d ", time.localtime())+tm+"]"}
                servers.append(sjson)
    return servers
...
if __name__ == "__main__":
    ...
    if shaPid < 0:
        os.system("START /B " + pFullPath)
    try: 
        ssserver = getFreeSSFromFile()#getFreeSS()
    except:
        print("获取账号失败！")
        print(e)
        ssserver = [] 
