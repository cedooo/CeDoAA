...
import cn.org.rapid_framework.generator2.util.AntPathMatcher;
import cn.org.rapid_framework.generator2.util.FileHelper;
import cn.org.rapid_framework.generator2.util.FreemarkerHelper;
import cn.org.rapid_framework.generator2.util.GeneratorException;
import cn.org.rapid_framework.generator2.util.IOHelper;
...
//		if(outRootDir == null) throw new IllegalStateException("'outRootDir' property must be not null.");
    ...
    public void deleteOutRootDir() throws IOException {
        ...
        cn.org.rapid_framework.generator2.util.GLogger.println("[delete dir]    "+getOutRootDir());
	...
	private void processTemplateRootDirs(Map templateModel,Map filePathModel,boolean isDelete) throws Exception {
		...
		GeneratorException ge = new GeneratorException("generator occer error, Generator BeanInfo:"+ cn.org.rapid_framework.generator2.util.BeanHelper.describe(this));
	...
	private List<Exception> scanTemplatesAndProcess(File templateRootDir, Map templateModel,Map filePathModel,boolean isDelete) throws Exception {
		...
		cn.org.rapid_framework.generator2.util.GLogger.println("-------------------load template from templateRootDir = '"+templateRootDir.getAbsolutePath()+"' outRootDir:"+new File(outRootDir).getAbsolutePath());
		...
		for(int i = 0; i < srcFiles.size(); i++) {
			...
			try {
				...
				if (ignoreTemplateGenerateException) {
			        cn.org.rapid_framework.generator2.util.GLogger.warn("iggnore generate error,template is:" + srcFile+" cause:"+e);
			        exceptions.add(e);
			    } else {
					throw e;
			    }
	...
	private class TemplateProcessor {
		private cn.org.rapid_framework.generator2.GeneratorControl gg = new cn.org.rapid_framework.generator2.GeneratorControl();
		private void executeGenerate(File templateRootDir,Map templateModel, Map filePathModel ,File srcFile) throws SQLException, IOException,TemplateException {
			...
			if(isCopyBinaryFile && FileHelper.isBinaryFile(srcFile)) {
				...
				cn.org.rapid_framework.generator2.util.GLogger.println("[copy binary file by extention] from:"+srcFile+" => "+new File(getOutRootDir(),outputFilepath));
            ...
            try {
                ...
                if(gg.isIgnoreOutput()) {
                    cn.org.rapid_framework.generator2.util.GLogger.println("[not generate] by gg.isIgnoreOutput()=true on template:"+templateFile);
                    return;
                }
	    ...
	    private void executeDelete(File templateRootDir,Map templateModel, Map filePathModel ,File srcFile) throws SQLException, IOException,TemplateException {
	        ...
	        processTemplateForGeneratorControl(templateModel, templateFile);
	        String outputFilepath = proceeForOutputFilepath(filePathModel, templateFile);
	        cn.org.rapid_framework.generator2.util.GLogger.println("[delete file] file:"+new File(gg.getOutRoot(),outputFilepath).getAbsolutePath());
		...
		private String proceeForOutputFilepath(Map filePathModel,String templateFile) throws IOException {
			...
			if((testExpressionIndex = templateFile.indexOf('@')) != -1) {
				...
				if(!"true".equals(String.valueOf(expressionValue))) {
					cn.org.rapid_framework.generator2.util.GLogger.println("[not-generate]\t test expression '@"+testExpressionKey+"' is false,template:"+templateFile);
						return null;
				}
			}
		...
		private void generateNewFileOrInsertIntoFile( String templateFile,String outputFilepath, Map templateModel) throws Exception {
			...
			if(absoluteOutputFilePath.exists()) {
				...
				if(GeneratorHelper.isFoundInsertLocation(gg,template, templateModel, absoluteOutputFilePath, newFileContentCollector)) {
					cn.org.rapid_framework.generator2.util.GLogger.println("[insert]\t generate content into:"+outputFilepath);
			...
			if(absoluteOutputFilePath.exists() && !gg.isOverride()) {
				cn.org.rapid_framework.generator2.util.GLogger.println("[not generate]\t by gg.isOverride()=false and outputFile exist:"+outputFilepath);
				return;
			}
			
			cn.org.rapid_framework.generator2.util.GLogger.println("[generate]\t template:"+templateFile+" ==> "+outputFilepath);
	...
	static class GeneratorHelper {
		public static boolean isIgnoreTemplateProcess(File srcFile,String templateFile,String includes,String excludes) {
			...
			if(srcFile.getName().toLowerCase().endsWith(".include")){
				cn.org.rapid_framework.generator2.util.GLogger.println("[skip]\t\t endsWith '.include' template:"+templateFile);
				return true;
			}
		...
		private static boolean isFoundInsertLocation(cn.org.rapid_framework.generator2.GeneratorControl gg, Template template, Map model, File outputFile, StringWriter newFileContent) throws IOException, TemplateException {
...
//			GLogger.info("[set Freemarker.autoIncludes]"+availableAutoInclude+" for templateName:"+templateName);
			...
			conf.setAutoIncludes(availableAutoInclude);
			cn.org.rapid_framework.generator2.util.GLogger.debug("set Freemarker.autoIncludes:"+availableAutoInclude+" for templateName:"+templateName+" autoIncludes:"+autoIncludes);
			return conf;
		}
