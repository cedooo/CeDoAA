...
import cn.org.rapid_framework.generator2.util.SqlExecutorHelper;
import cn.org.rapid_framework.generator2.util.SystemHelper;
...
public class GeneratorControl {
	...
	public NodeModel loadXml(String file,boolean removeXmlNamespace) {
		try {
			if(removeXmlNamespace) {
				...
				String encoding = cn.org.rapid_framework.generator2.util.XMLHelper.getXMLEncoding(forEncodingInput);
				forEncodingInput.close();
				
				InputStream input = FileHelper.getInputStream(file);
				String xml = IOHelper.toString(encoding,input);
				xml = cn.org.rapid_framework.generator2.util.XMLHelper.removeXmlns(xml);
				input.close();
	...
	public void generateFile(String outputFile,String content,boolean append) {
		try {
			...
			if(deleteGeneratedFile) {
				cn.org.rapid_framework.generator2.util.GLogger.println("[delete gg.generateFile()] file:"+realOutputFile+" by template:"+getSourceFile());
				new File(realOutputFile).delete();
			}else {
				File file = new File(realOutputFile);
				FileHelper.parnetMkdir(file);
				cn.org.rapid_framework.generator2.util.GLogger.println("[gg.generateFile()] outputFile:"+realOutputFile+" append:"+append+" by template:"+getSourceFile());
				IOHelper.saveFile(file, content,getOutputEncoding(),append);
			}
		} catch (Exception e) {
			cn.org.rapid_framework.generator2.util.GLogger.warn("gg.generateFile() occer error,outputFile:"+outputFile+" caused by:"+e,e);
	...
	public String getProperty(String key,String defaultValue){
		return cn.org.rapid_framework.generator2.GeneratorProperties.getProperty(key, defaultValue);
	}
...
//	}
	...
	public String getInputProperty(String key,String message) throws IOException {
		String v = cn.org.rapid_framework.generator2.GeneratorProperties.getProperty(key);
		if(v == null) {
			...
			cn.org.rapid_framework.generator2.GeneratorProperties.setProperty(key, v);
		}
		return v;
	}
